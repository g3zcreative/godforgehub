...
function RecommendationSection({ title, icon, items, linkPrefix, emptyText }: {
  title: string;
  ...
  return (
    <div>
      <h2 className="text-base font-display font-semibold mb-3 flex items-center gap-2">{icon} {title}</h2>
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((item: any) => (
            ...
            >
              {item.image_url && (
                <img src={item.image_url} alt={item.name} className="h-10 w-10 rounded object-cover border border-border group-hover:border-primary/40 transition-colors" />
              )}
              <div className="flex-1 min-w-0">
